...
Now you have an installed and working numahop instance. But the base
installation of numahop is rather empty. The first thing you must create before
anything else is the libraries declared inside the yaml configuration files.
...
There is a default account with login `admin`. This account should only be used
to create new users and the administration tools.

## Adding a Library

There should already be a library present with identifier `lib_library`. This
one can be used. If you want more or one with another identifer it should be
configured right after installation. Each library must be referenced in the
database and its identifier registered in the config file.

For each libraries declared create a library through the interface. Then you
need to edit the libraries inside the database to make the column `identifier`
match the names declared inside the yaml files. If the library you created
isn't defined in the yaml file you need to edit the yaml file and add an
identifer to it and restart numahop.
...
```
Once the libraries are created you can create the rest of the objects you need
for your use of NumaHOP.


If you have created other objects before the library column update by accident here is how
to fix it:
```SQL
...
I advise you create a admin user profile with all rights allowed for each
libraries as the admin account configured in the yaml file can do some of the
things but there are a lot of functionality that are disallowed on this
account.
